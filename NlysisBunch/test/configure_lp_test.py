...
debug = "-Dcom.sun.management.jmxremote.port=%d -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false" % (
port)
# debug = ""
cmd = "%s -Xms20m -Xmx1024m %s -Djava.endorsed.dirs=%s/Resources/Dsz/Gui/Lib/endorsed -jar %s/Start.jar %s %s" % (
javaExe, debug, scriptDir, scriptDir, args, eol)
